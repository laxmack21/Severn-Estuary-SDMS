...
env_uk<-stack(depth,tidal_range)
...
writeRaster(pred_PredFull, filename="pred8_me Pout.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
...
depth_Barrage<-raster("Future Data Pout/Barrage/bathy_masked.tif")
tidal_range_Barrage<-raster("Future Data Pout/Barrage/tidal_range_masked.tif")
#Stack raster layers into one file
env_Barrage<-stack(tidal_range_Barrage,depth_Barrage)
...
plot(pred_barrage)
writeRaster(pred_barrage, filename="pred_barrage Pout.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
...
depth_Lagoon<-raster("Future Data Pout/Cardiff_Lagoon/bathy_masked.tif")
tidal_range_Lagoon<-raster("Future Data Pout/Cardiff_Lagoon/tidal_range_masked.tif")
#Raster stack commence for lagoon scenariooo
env_Lagoon<-stack(depth_Lagoon,tidal_range_Lagoon)

# Now, predict a maxent model using the "trained model" and the environmental data for the Barrage Scenario. This may take a few moments to run:
pred_lagoon = predict(model, env_Lagoon,progress='text')
plot(pred_lagoon)
writeRaster(pred_lagoon, filename="pred_lagoon Pout.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
#CANT SEE RESPONSE CURVES FOR THIS MODEL? -maybe dont need to either?
