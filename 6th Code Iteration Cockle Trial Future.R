...
env_uk<-stack(depth,min_elev,dry_always,subtidal)
...
writeRaster(pred_PredFull, filename="pred6_me Cockle.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
...
depth_Barrage<-raster("Future Data Cockle/Barrage/bathy_masked.tif")
min_fs_Barrage<-raster("Future Data Cockle/Barrage/min_elev_masked.tif")
subtidal_Barrage<-raster("Future Data Cockle/Barrage/subtidal_masked.tif")
always_dry_Barrage<-raster("Future Data Cockle/Barrage/always_dry_masked.tif")
#Stack raster layers into one file
env_Barrage<-stack(subtidal_Barrage,depth_Barrage,min_fs_Barrage,always_dry_Barrage)
...
plot(pred_barrage)
writeRaster(pred_barrage, filename="pred_barrage Cockle 6TRIAL.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
#CANT SEE RESPONSE CURVES FOR THIS MODEL? -maybe dont need to either?

--
  
#Load env Data for Cardiff Lagoon Scenario
depth_Lagoon<-raster("Future Data Cockle/Cardiff_Lagoon/bathy_masked.tif")
min_fs_Lagoon<-raster("Future Data Cockle/Cardiff_Lagoon/min_elev_masked.tif")
subtidal_Lagoon<-raster("Future Data Cockle/Cardiff_Lagoon/subtidal_masked.tif")
always_dry_Lagoon<-raster("Future Data Cockle/Cardiff_Lagoon/always_dry_masked.tif")
#Raster stack commence for lagoon scenariooo
env_Lagoon<-stack(depth_Lagoon,min_fs_Lagoon,subtidal_Lagoon,always_dry_Lagoon)

# Now, predict a maxent model using the "trained model" and the environmental data for the Barrage Scenario. This may take a few moments to run:
pred_lagoon = predict(model, env_Lagoon,progress='text')
plot(pred_lagoon)
writeRaster(pred_lagoon, filename="pred_lagoon Cockle 6TRIAL.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
#CANT SEE RESPONSE CURVES FOR THIS MODEL? -maybe dont need to either?
