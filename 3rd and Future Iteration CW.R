...
setwd("~/Documents/UoY/Dissertation/Common Whelk")
locs = read.csv("Common_Welk_Severn_UTM.csv", header=T, sep = ",")

#loading severn files
#8 Variables (-always dry) + MASK2?
tidal_range<-raster("Severn_unaltered CW/tidal_range_masked.tif")
subtidal<-raster("Severn_unaltered CW/subtidal_masked.tif")
min_elev<-raster("Severn_unaltered CW/min_elev_masked.tif")
max_velocity<-raster("Severn_unaltered CW/max_vel_masked.tif")
max_elev<-raster("Severn_unaltered CW/max_elev_masked.tif")
mask_2<-raster("Severn_unaltered CW/mask2.tif")
depth<-raster("Severn_unaltered CW/bathy_masked.tif")
intertidal<-raster("Severn_unaltered CW/intertidal_masked.tif")
avg_velocity<-raster("Severn_unaltered CW/av_vel_masked.tif")
mask<-depth
...
pred_PredFull2 <- predict(model, env_uk)
...
writeRaster(pred_PredFull2, filename="pred2_me.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
...
depth_Barrage<-raster("Future Data CW/Barrage/bathy_masked.tif")
max_fs_Barrage<-raster("Future Data CW/Barrage/max_elev_masked.tif")
max_vel_Barrage<-raster("Future Data CW/Barrage/max_vel_masked.tif")
min_fs_Barrage<-raster("Future Data CW/Barrage/min_elev_masked.tif")
mean_vel_Barrage<-raster("Future Data CW/Barrage/av_vel_masked.tif")
subtidal_Barrage<-raster("Future Data CW/Barrage/subtidal_masked.tif")
intertidal_Barrage<-raster("Future Data CW/Barrage/intertidal_masked.tif")
tidal_range_Barrage<-raster("Future Data CW/Barrage/tidal_range_masked.tif")
#Stack raster layers into one file - WILL NEED TO CHECK TO MAKESURE THE RIGHT LAYERS ARE INCLUDED
env_Barrage<-stack(tidal_range_Barrage,subtidal_Barrage,depth_Barrage,max_fs_Barrage,max_vel_Barrage,mean_vel_Barrage,min_fs_Barrage,intertidal_Barrage)
...
depth_Lagoon<-raster("Future Data CW/Cardiff_Lagoon/bathy_masked.tif")
max_fs_Lagoon<-raster("Future Data CW/Cardiff_Lagoon/max_elev_masked.tif")
max_vel_Lagoon<-raster("Future Data CW/Cardiff_Lagoon/max_vel_masked.tif")
min_fs_Lagoon<-raster("Future Data CW/Cardiff_Lagoon/min_elev_masked.tif")
mean_vel_Lagoon<-raster("Future Data CW/Cardiff_Lagoon/av_vel_masked.tif")
subtidal_Lagoon<-raster("Future Data CW/Cardiff_Lagoon/subtidal_masked.tif")
intertidal_Lagoon<-raster("Future Data CW/Cardiff_Lagoon/intertidal_masked.tif")
tidal_range_Lagoon<-raster("Future Data CW/Cardiff_Lagoon/tidal_range_masked.tif")
#Raster stack commence for lagoon scenariooo
env_Lagoon<-stack(depth_Lagoon,max_fs_Lagoon,max_vel_Lagoon,min_fs_Lagoon,mean_vel_Lagoon,subtidal_Lagoon,tidal_range_Lagoon,intertidal_Lagoon)
