...
env_uk<-stack(depth,subtidal,min_elev)
...
writeRaster(pred_PredFull, filename="pred7_me Cockle.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
...
env_Barrage<-stack(subtidal_Barrage,depth_Barrage,min_fs_Barrage)
...
plot(pred_barrage)
writeRaster(pred_barrage, filename="pred_barrage Cockle.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
...
env_Lagoon<-stack(depth_Lagoon,min_fs_Lagoon,subtidal_Lagoon)

# Now, predict a maxent model using the "trained model" and the environmental data for the Barrage Scenario. This may take a few moments to run:
pred_lagoon = predict(model, env_Lagoon,progress='text')
plot(pred_lagoon)
writeRaster(pred_lagoon, filename="pred_lagoon Cockle.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
#CANT SEE RESPONSE CURVES FOR THIS MODEL? -maybe dont need to either?
