...
setwd("~/Documents/UoY/Dissertation/Pouting 2")
locs = read.csv("Pouting 2_Severn_UTM.csv", header=T, sep = ",")

#loading severn files
#had to add the file location for R to access the severn files, is this right?
dry_always<-raster("Severn_unaltered Pouting 2/always_dry_masked.tif")
tidal_range<-raster("Severn_unaltered Pouting 2/tidal_range_masked.tif")
subtidal<-raster("Severn_unaltered Pouting 2/subtidal_masked.tif")
min_elev<-raster("Severn_unaltered Pouting 2/min_elev_masked.tif")
max_velocity<-raster("Severn_unaltered Pouting 2/max_vel_masked.tif")
max_elev<-raster("Severn_unaltered Pouting 2/max_elev_masked.tif")
mask_2<-raster("Severn_unaltered Pouting 2/mask2.tif")
intertidal<-raster("Severn_unaltered Pouting 2/intertidal_masked.tif")
depth<-raster("Severn_unaltered Pouting 2/bathy_masked.tif")
avg_velocity<-raster("Severn_unaltered Pouting 2/av_vel_masked.tif")
...
env_uk<-stack(depth,max_elev,max_velocity,min_elev,avg_velocity,intertidal,subtidal,tidal_range)
...
writeRaster(pred_PredFull, filename="pred3.2_me.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
...
depth_Barrage<-raster("Future Data Pouting 2/Barrage/bathy_masked.tif")
max_fs_Barrage<-raster("Future Data Pouting 2/Barrage/max_elev_masked.tif")
max_vel_Barrage<-raster("Future Data Pouting 2/Barrage/max_vel_masked.tif")
min_fs_Barrage<-raster("Future Data Pouting 2/Barrage/min_elev_masked.tif")
mean_vel_Barrage<-raster("Future Data Pouting 2/Barrage/av_vel_masked.tif")
intertidal_Barrage<-raster("Future Data Pouting 2/Barrage/intertidal_masked.tif")
subtidal_Barrage<-raster("Future Data Pouting 2/Barrage/subtidal_masked.tif")
tidal_range_Barrage<-raster("Future Data Pouting 2/Barrage/tidal_range_masked.tif")
#Stack raster layers into one file
env_Barrage<-stack(tidal_range_Barrage,depth_Barrage,max_fs_Barrage,max_vel_Barrage,mean_vel_Barrage,min_fs_Barrage,intertidal_Barrage,subtidal_Barrage)
...
plot(pred_barrage)
writeRaster(pred_barrage, filename="pred_barrage Pouting 2.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
...
depth_Lagoon<-raster("Future Data Pouting 2/Cardiff_Lagoon/bathy_masked.tif")
max_fs_Lagoon<-raster("Future Data Pouting 2/Cardiff_Lagoon/max_elev_masked.tif")
max_vel_Lagoon<-raster("Future Data Pouting 2/Cardiff_Lagoon/max_vel_masked.tif")
min_fs_Lagoon<-raster("Future Data Pouting 2/Cardiff_Lagoon/min_elev_masked.tif")
mean_vel_Lagoon<-raster("Future Data Pouting 2/Cardiff_Lagoon/av_vel_masked.tif")
tidal_range_Lagoon<-raster("Future Data Pouting 2/Cardiff_Lagoon/tidal_range_masked.tif")
subtidal_Lagoon<-raster("Future Data Pouting 2/Cardiff_Lagoon/subtidal_masked.tif")
intertidal_Lagoon<-raster("Future Data Pouting 2/Cardiff_Lagoon/intertidal_masked.tif")
#Raster stack commence for lagoon scenariooo
env_Lagoon<-stack(depth_Lagoon,max_fs_Lagoon,max_vel_Lagoon,min_fs_Lagoon,mean_vel_Lagoon,tidal_range_Lagoon,intertidal_Lagoon,subtidal_Lagoon)

# Now, predict a maxent model using the "trained model" and the environmental data for the Barrage Scenario. This may take a few moments to run:
pred_lagoon = predict(model, env_Lagoon,progress='text')
plot(pred_lagoon)
writeRaster(pred_lagoon, filename="pred_lagoon Pouting 2.tif", options="INTERLEAVE=BAND", overwrite=TRUE)
#CANT SEE RESPONSE CURVES FOR THIS MODEL? -maybe dont need to either?
